...
# =========================
ENV_ID         = "ALE/Pong-v5"
TOTAL_STEPS    = 20_000_000          # tune up for serious training (e.g., 10–20M)
N_ENVS         = 16
...
def main():
    ...
    while global_steps < TOTAL_STEPS:
        ...
        # -------- Returns & advantages --------
        advantages, returns  = compute_gae(rew_buf, done_buf, val_buf, last_v, GAMMA, GAE_LAMBDA)                  # [T,N]
        ...
        if cur_step % 100_000 == 0:
            ...
            # Reward
            plt.plot(hist_steps, hist_rmean, label="Reward (mean100)")

            # Losses (scaled to comparable range if needed)
            plt.plot(hist_steps, hist_pi_loss, label="Policy loss (MA)", alpha=0.9)
            plt.plot(hist_steps, hist_v_loss, label="Value loss (MA)", alpha=0.9)

            # Entropy (helps diagnose collapse)
            plt.plot(hist_steps, hist_ent, label="Entropy (MA)", alpha=0.9)

            plt.xlabel("Env steps")
            plt.legend()
            plt.title("A2C Training Curves")
